...
echo "==> Building and starting containers (uid=$DOCKER_UID, gid=$DOCKER_GID)..."
sudo -u stockagent bash -c "cd $APP_DIR && DOCKER_UID=$DOCKER_UID DOCKER_GID=$DOCKER_GID docker compose up -d --build"

echo "==> Waiting for container to start..."
sleep 8

echo "==> Checking status..."
if sudo -u stockagent bash -c "cd $APP_DIR && DOCKER_UID=$DOCKER_UID DOCKER_GID=$DOCKER_GID docker compose ps --status running" 2>/dev/null | grep -q stock-agent; then
  echo "SUCCESS: Container is running."
  echo ""
  echo "==> Recent logs:"
  sudo -u stockagent bash -c "cd $APP_DIR && DOCKER_UID=$DOCKER_UID DOCKER_GID=$DOCKER_GID docker compose logs --tail 20"
else
  echo "FAILED: Container is not running."
  echo ""
  echo "==> Error logs:"
  sudo -u stockagent bash -c "cd $APP_DIR && DOCKER_UID=$DOCKER_UID DOCKER_GID=$DOCKER_GID docker compose logs --tail 30"
  exit 1
fi
